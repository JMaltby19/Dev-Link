	...
	(_id: string | undefined) => async (dispatch: Dispatch<ActionTypes>) => {
		...
		try {
			const res = await axios.delete<ProfileType>(
				`http://localhost:6001/api/profile/experience/${_id}`,
				{
					headers: {
						"Content-Type": "application/json",
					},
...
export const deleteEducation =
	(_id: string | undefined) => async (dispatch: Dispatch<ActionTypes>) => {
		try {
			const res = await axios.delete<ProfileType>(
				`http://localhost:6001/api/profile/education/${_id}`,
				{
					headers: {
						"Content-Type": "application/json",
					},
...
export const deleteAccount = () => async (dispatch: Dispatch<ActionTypes>) => {
	if (window.confirm("Are you sure ? This action cannot be undone")) {
		try {
			await axios.delete<ProfileType>("http://localhost:6001/api/profile");
			dispatch({ type: CLEAR_PROFILE });
